...
class PHPMailer
{
    ...
    public static function ValidateAddress($address)
    {
        if (
            ...
            version_compare(PCRE_VERSION, '8.0') >= 0
        ) {
            return preg_match(
        ...
        try {
            ...
            if ($this->Mailer == 'mail') {
                ...
                $this->mailHeader .= $this->HeaderLine(
                    'Subject',
                    $this->EncodeHeader(
                        $this->SecureHeader(trim($this->Subject))
                    )
                );
                // if(count($this->cc) > 0) {
    ...
    protected function SmtpSend($header, $body)
    {
        ...
        if (count($bad_rcpt) > 0) {
            ...
            throw new phpmailerException(
                $this->Lang('recipients_failed') . $badaddresses
            );
        }
    ...
    public function SmtpConnect()
    {
        ...
        try {
            while ($index < count($hosts) && !$connection) {
                $hostinfo = [];
                if (
                    preg_match('/^(.+):([0-9]+)$/', $hosts[$index], $hostinfo)
                ) {
                    $host = $hostinfo[1];
                    $port = $hostinfo[2];
                } else {
...
//            }
    ...
    protected function SetError($msg)
    {
        ...
        if ($this->Mailer == 'smtp' and !is_null($this->smtp)) {
            ...
            if (
                !empty($lasterror) and array_key_exists('smtp_msg', $lasterror)
            ) {
                $msg .=
                    '<p>' .
    ...
    public function MsgHTML($message, $basedir = '')
    {
        ...
        if (empty($this->AltBody)) {
            ...
            if (!empty($textMsg)) {
                $this->AltBody = html_entity_decode(
                    $textMsg,
                    ENT_QUOTES,
                    $this->CharSet
                );
    ...
    public static function _mime_types($ext = '')
    {
        ...
        return !isset($mimes[strtolower($ext)])
